...
let vue = new window.Vue({
    ...
    computed: {
        ...
        getCurrentNode() {
            if (this.currentNode) {
                ...
                let nodeColorKey = previewKey.nodeColorKey;
                if (!this.currentNode[nodeColorKey]) {
                    cc.js.getset(this.currentNode, nodeColorKey, () => {
                        return '#' + this.currentNode.color.toHEX();
                    }, (hex) => {
                        if (hex == null) {
                            hex = '#FFFFFF'
                        }
                        this.currentNode.color = new cc.Color(hex).fromHEX(hex);
                    }, false, true);
                }

                let cameraColorKey = previewKey.cameraColorKey;
                let hasCamera = this.currentNode.getComponent(cc.Camera);
                if (hasCamera && !hasCamera[cameraColorKey]) {
                    cc.js.getset(hasCamera, cameraColorKey, () => {
                        return '#' + this.currentNode.getComponent(cc.Camera)?.backgroundColor.toHEX();
                    }, (hex) => {
                        let temp = this.currentNode.getComponent(cc.Camera);
                        if (temp) {
                            temp.backgroundColor = new cc.Color(hex).fromHEX(hex);
                        }
    ...
    methods: {
        ...
        clickNodeItem(node) {
            ...
            if (node?.node?._components) {
                for (let comp of node.node._components) {
                    let name = this.getCompName(comp.name);
                    if (typeof name != 'string') {
                        name = '-InvalidCompName-'
                    }
                    let o2 = { name: name, index: count + '' };
                    compMap[count] = comp;
        ...
        getCompValues(compName, item) {
            let compTemplate = window.preview_config.compTemplate;
            let values = compTemplate[compName]?.values;
            return values || [];
        },
